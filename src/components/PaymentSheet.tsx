...
const BENEFITS = ['Full sessions — 5 min to all night', 'Every theme & chant', 'Unlimited sleep timer', 'Offline caching']
...
export function PaymentSheet() {
  ...
  return (
    <Sheet open={paymentOpen} onClose={close} title={status === 'success' ? undefined : 'Drift Premium'}>
      {status === 'success' ? (
        <div style={{ textAlign: 'center', padding: '12px 0 8px' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: 15, margin: 0 }}>Welcome to Drift Premium.</p>
        </div>
      ) : (
        <>
          {/* order summary */}
          <div style={summary}>
            <div>
              <div style={{ fontSize: 16, color: 'var(--text-primary)' }}>Drift Premium</div>
